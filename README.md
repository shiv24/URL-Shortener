...
* Storage of Analytics: Each short url redirection timestamp is appended to an array within a mongo document identified by the short key. In Mongo the max size of a document is 16MB so for more frequently accessed url's this document could fill up fast. 
